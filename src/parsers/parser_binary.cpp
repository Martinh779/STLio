...
namespace stlio {
    ...
    STLModel ParserBinary::read(const std::string &filename) {
        ...
        vector<Face> faces(num_triangles);
        for (int i = 0; i < num_triangles; i++) {
            std::memcpy(&faces[i].normal, &bytes[84 + i * 50], sizeof(Vector3D));
            Triangle &triangle = faces[i].triangle;
            std::memcpy(&triangle.v1, &bytes[96 + i * 50], sizeof(Vector3D));
            std::memcpy(&triangle.v2, &bytes[108 + i * 50], sizeof(Vector3D));
            std::memcpy(&triangle.v3, &bytes[120 + i * 50], sizeof(Vector3D));
        }
        STLModel stl_model;
    ...
    void ParserBinary::write(const std::string &filename, STLModel stlModel) {
        ...
        // modify buffer
        fillBufferWithModel(buffer, stlModel);

        // write to file
        stl_file.write(buffer, size);
        stl_file.close();
    }
    ...
    void ParserBinary::fillBufferWithModel(char* buffer, STLModel stlModel) {
        ...
        vector<Face> faces = stlModel.getFaces();
        ...
        for (int i = 0; i < numOfFaces; i++) {
            std::memcpy(&buffer[offset_bytes + i * FACE_BYTES], &faces[i], sizeof(Face));
        }
